...
| `2026_08_19_add_cargo_tracking_index.sql` | افزودن `idx_cargo_tracking`/`idx_cargo_ship_tracking` روی `CargoInfo` (DEEP_CODE_AUDIT.md) | نوشته شده و روی MariaDB throwaway با ۱۰٬۰۰۰ ردیف تست شد (`EXPLAIN`: type از `ALL`+filesort به `ref`/`range` بدون filesort رسید) — **هنوز روی دیتابیس تولید اجرا نشده**؛ بدون downtime قابل اجراست (InnoDB online DDL) |
...
| `2026_08_22_create_monitoring_events_table.sql` | ساخت جدول `monitoring_events` برای هشدارهای pull-based (سرور دسترسی خروجی اینترنت ندارد، Telegram هرگز واقعاً کار نمی‌کند) — DEEP_CODE_AUDIT.md #۳۲ | `CREATE TABLE IF NOT EXISTS` — بی‌خطر اگر جدول از قبل وجود داشته باشد. باید هم‌زمان با deploy `SecurityAlerter`/`MonitoringController`/route‌های `monitoring/*` اجرا شود، وگرنه `MonitoringEventLogger::record()` بی‌صدا شکست می‌خورد (مثل سرنوشت اولیه‌ی `audit_log`) و endpointهای `monitoring/events`/`monitoring/summary` با خطای «جدول موجود نیست» ۵۰۰ می‌دهند |
| `2026_08_22_add_initial_info_unique_key.sql` | افزودن `UNIQUE KEY uk_initial_natural` روی کلید طبیعی `InitialInfo` — پیش‌شرط رفع باگ بیش‌شماری JOIN (DEEP_CODE_AUDIT.md #۳۷، مرحله ۱ از ۲) | نوشته شده، **هنوز روی هیچ دیتابیسی اجرا نشده**. قبل از اجرا حتماً کوئری تشخیص تکراری بالای فایل را بزنید — اگر ردیفی برگرداند، `ALTER TABLE` با خطای Duplicate entry (بی‌خطر، atomic) رد می‌شود تا داده‌ی تکراری را دستی بررسی/ادغام کنید |
| `2026_08_22_add_cargo_info_initial_info_fk.sql` | افزودن `initial_info_id` به `CargoInfo`، backfill از کلید طبیعی، و FK واقعی به `InitialInfo.id` با `ON DELETE CASCADE` (هم‌رفتار با `QuotaService::deleteQuota` موجود) — DEEP_CODE_AUDIT.md #۳۷، مرحله ۲ از ۲ | نوشته شده، **هنوز روی هیچ دیتابیسی اجرا نشده**. **پیش‌نیاز اجباری:** migration مرحله ۱ (بالا) باید قبلاً با موفقیت اجرا شده باشد، وگرنه backfill می‌تواند غیرقطعی شود. هم‌زمان با همین commit، `CargoRepository::insertCargo` نیز `initial_info_id` را برای رکوردهای جدید پر می‌کند |
| `2026_08_24_create_db_optimization_history_table.sql` | ساخت جدول `db_optimization_history` برای پنل جدید `PHP/MySQL_Manager` (فاز ۲: OPTIMIZE/ANALYZE/CHECK/REPAIR TABLE) | `CREATE TABLE IF NOT EXISTS` — بی‌خطر اگر جدول از قبل وجود داشته باشد. باید هم‌زمان با deploy `DatabaseOptimizationService`/`MySQL_Manager/api.php` اجرا شود، وگرنه ثبت تاریخچه‌ی عملیات بی‌صدا شکست می‌خورد (خودِ عملیات OPTIMIZE/ANALYZE/CHECK/REPAIR روی MySQL باز هم اجرا و به کاربر گزارش می‌شود، فقط تاریخچه ثبت نمی‌شود) |
| `2026_08_25_create_mysql_manager_backup_tables.sql` | ساخت جدول‌های `db_backups`/`db_maintenance_settings`/`db_cleanup_rules` برای پنل `PHP/MySQL_Manager` (فاز ۳: Backup/Restore/Cleanup) | `CREATE TABLE IF NOT EXISTS` — بی‌خطر. یک ردیف پیش‌فرض در `db_maintenance_settings` درج می‌شود (`INSERT IGNORE`، id=1) تا کد همیشه یک ردیف تنظیمات پیدا کند. `db_cleanup_rules` عمداً خالی می‌ماند — بدون Rule صریح و فعال هیچ پاکسازی‌ای اجرا نمی‌شود. باید هم‌زمان با deploy کد این فاز و ساخت پوشه‌ی `PHP/storage/mysql_backups/` (با `.htaccess` مسدودکننده‌ی دسترسی مستقیم HTTP) اجرا شود |
