<?php
// PHP/Quota Reports/login.php ورود به پنل گزارش آماری کوتاژ.
...
$error = '';
$passwordHash = (string)Config::getInstance()->get('quota_reports_admin_password_hash', '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!Csrf::validate($_POST['csrf_token'] ?? null)) {
        ...
        $error = 'خطای پیکربندی: متغیر QUOTA_REPORTS_ADMIN_PASSWORD_HASH در فایل .env تنظیم نشده است.';
        $logger->security('Quota Reports panel login attempted while QUOTA_REPORTS_ADMIN_PASSWORD_HASH is unset');
    } elseif ($limiter->isLocked(QR_ACTOR, $clientIp)) {
        ...
        $limiter->resetAttempts(QR_ACTOR, $clientIp);

        session_regenerate_id(true);
        $_SESSION[QR_SESSION_KEY] = true;
        ...
        $logger->security("Quota Reports panel login failed from {$clientIp}");
        $error = 'رمز عبور نادرست است.';
    }
}
...
<?php require __DIR__ . '/_icons.php'; ?>
    ...
    <main class="auth-card">
        <div class="auth-brand">
            <svg class="icon icon-lg" aria-hidden="true"><use href="#chart"></use></svg>
            <div>
                <h1>گزارش آماری کوتاژ</h1>
