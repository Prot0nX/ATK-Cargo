<?php
// PHP/MySQL_Manager/login.php ورود به پنل مدیریت MySQL.
...
$error = '';
$passwordHash = (string)Config::getInstance()->get('mysql_manager_admin_password_hash', '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!Csrf::validate($_POST['csrf_token'] ?? null)) {
        ...
        $error = 'خطای پیکربندی: متغیر MYSQL_MANAGER_ADMIN_PASSWORD_HASH در فایل .env تنظیم نشده است.';
        $logger->security('MySQL Manager panel login attempted while MYSQL_MANAGER_ADMIN_PASSWORD_HASH is unset');
    } elseif ($limiter->isLocked(MYS_ACTOR, $clientIp)) {
        ...
        $limiter->resetAttempts(MYS_ACTOR, $clientIp);

        session_regenerate_id(true);
        $_SESSION[MYS_SESSION_KEY] = true;
        $_SESSION['last_activity'] = time();
        Csrf::token();

        $logger->security("MySQL Manager panel login successful from {$clientIp}");
        header('Location: index.php');
        exit;
    } else {
        $limiter->registerFailedAttempt(MYS_ACTOR, $clientIp);
        $logger->security("MySQL Manager panel login failed from {$clientIp}");
        $error = 'رمز عبور نادرست است.';
    }
}
...
<?php require __DIR__ . '/_icons.php'; ?>
    ...
    <main class="auth-card">
        <div class="auth-brand">
            <svg class="icon icon-lg" aria-hidden="true"><use href="#database"></use></svg>
            <div>
                <h1>مدیریت MySQL</h1>
