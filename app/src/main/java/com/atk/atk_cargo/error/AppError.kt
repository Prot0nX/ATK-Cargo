...
// نگاشت خطای متمرکز؛ هنوز به ViewModel/Screen سیم‌کشی نشده (پیام‌های context-specific فعلی حفظ می‌شوند)، اما catch(Exception) در ۹ فایل شبکه اصلاح شد تا CancellationException به‌اشتباه «خطای سرور» نمایش داده نشود — این فایل برای استفاده‌ی تدریجی/آینده نگه داشته شده است
