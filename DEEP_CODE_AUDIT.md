...
| 4.3 | ⚠️ در حال انجام — `feature:auth` (Phase 5.10) و `feature:startup` (Phase 5.11) + `core:domain` (پیش‌نیاز، Phase 5.11) استخراج شدند؛ ۱۰ feature باقی‌مانده هنوز در `app` هستند و اکثرشان (`cargo_*`, `reports`, `chat`) به‌شدت به‌هم و به `CargoViewModel`/`ui/viewmodel` (God ViewModel) وابسته‌اند — کشف شد حین بررسی `cargo_details` به‌عنوان کاندید بعدی (پایین). جزئیات در پایین | High |
