...
| ۳۲ | وصل کردن `health_monitor.php` به مانیتورینگ — بازطراحی‌شده به مدل pull-based (سرور دسترسی خروجی اینترنت ندارد، Telegram اصولاً ممکن نیست) | Medium | ✅ هر سه فاز اعمال شد — بک‌اند (الف)، داشبورد وب (ب)، بخش نظارت در اپ اندروید (ج) |
...
**یادداشت‌های اجرای مورد ۳۲ (فاز ب — داشبورد وب):**

- ساختار پنل جدید `PHP/Monitoring/` عیناً کپی الگوی `PHP/Lic/` است (همان تفکیک `_guard.php`/`login.php`/`logout.php`/`index.php`/`api.php`/`_icons.php`/`assets/`) — نه یک الگوی تازه. `assets/app.css` و فونت‌های Vazirmatn مستقیماً از `Lic` کپی شدند (همان توکن‌های رنگ/فاصله) تا هر دو پنل یک زبان بصری داشته باشند؛ فقط کلاس‌های مخصوص فرم لایسنس حذف و badgeهای شدت/سلامت تازه اضافه شدند.
- **تصمیم کلیدی طراحی:** این داشبورد یک رمز عبور مشترک مستقل دارد (`MONITORING_ADMIN_PASSWORD_HASH`، هش bcrypt در `.env`)، **نه** ورود با نام‌کاربری/رمز واقعی کاربران اپ از طریق `api/v2/auth/login`. دلیل: `SessionService` هر کاربر را فقط روی یک دستگاه هم‌زمان مجاز می‌داند (`user_sessions` یک ردیف به‌ازای هر `username+deviceId`) — اگر این داشبورد از همان مکانیزم نشست اپ استفاده می‌کرد، ورود یک ادمین از مرورگر نشست موبایل همان ادمین را می‌شکست، دقیقاً در لحظه‌ای که این داشبورد قرار است استفاده شود (زمانی که یک ادمین با اپ کار می‌کند). به همین دلیل `api/v2/monitoring/*` (فاز الف) از این پنل استفاده نمی‌شود؛ `Monitoring/api.php` مستقیماً `MonitoringController` را in-process صدا می‌زند — دقیقاً همان الگوی `Lic/api.php` با `LicenseAdminService`.
- نشست این پنل کاملاً مستقل است: `session_name('ATKMON')` (سومین نام نشست مجزا در کنار `PermissionManager`ی پیش‌فرض و `ATKLIC`)، actor مشترک `monitoring_admin` (شبیه `LIC_ACTOR`) که در `acknowledged_by` ثبت می‌شود — یعنی این ستون هویت فردی نیست، فقط نشان می‌دهد از طریق این پنل تأیید شده.
- برخلاف `Lic/assets/app.js` که بازخوانی خودکار را عمداً حذف کرده بود (چون نشست را برای همیشه زنده نگه می‌داشت)، این داشبورد هر ۳۰ ثانیه خودکار بازخوانی می‌کند — چون کل هدف فاز الف همین «pull بجای push» بود؛ حذف بازخوانی خودکار داشبورد را برای مانیتورینگ واقعی بی‌فایده می‌کرد. انقضای بی‌کاری نشست (پیش‌فرض ۳۰ دقیقه) هنوز برقرار است، فقط بازخوانی داده هر بار `last_activity` را هم تازه می‌کند.
- `PHP/phpstan.neon` به‌روزرسانی شد تا مسیر `Monitoring` را هم مثل `Lic` آنالیز کند (طبق کامنت خودِ فایل: «پنل‌های مدیریتی کد واقعی‌اند و باید مثل بقیه آنالیز شوند»). یک خطای واقعی PHPStan در `_guard.php` (نوع `session_name(): string|false` و `samesite` غیر-literal از `session_get_cookie_params()`) با cast/allow-list صریح رفع شد، نه ignore — کد Lic دقیقاً همین مشکل را دارد اما چون قدیمی‌تر از معرفی PHPStan به پروژه است در baseline پنهان مانده.
- تأیید شد: `php -l` روی همه‌ی فایل‌های جدید، `phpstan analyse` کل پروژه (شامل `Monitoring/` تازه) بدون خطا، `phpunit` بدون رگرسیون (۱۱۲/۱۱۲). یک smoke test end-to-end با سرور توکار PHP (`php -S`) و یک `MONITORING_ADMIN_PASSWORD_HASH` موقت اجرا شد: بدون نشست → ریدایرکت ۳۰۲ به `login.php` (و ۴۰۱ JSON برای `api.php`)؛ POST بدون CSRF → رد با پیام صحیح؛ ورود موفق با رمز درست → ۳۰۲ و کوکی `ATKMON` معتبر → `index.php` با توکن CSRF درست رندر شد؛ همه‌ی فایل‌های استاتیک (`app.css`/`app.js`/فونت‌ها/favicon) ۲۰۰ برگرداندند. `api.php?action=summary/events` بعد از ورود موفق ۵۰۰ برگرداند چون این محیط به دیتابیس واقعی وصل نیست (`Unknown database 'atk_cargo'`) — همان محدودیت فاز الف، نه یک باگ در این فاز؛ لایه‌ی auth/session/CSRF کامل تأیید شد، فقط لایه‌ی داده به دیتابیس واقعی نیاز دارد که قبل از deploy باید دستی تأیید شود.

**یادداشت‌های اجرای مورد ۳۲ (فاز ج — بخش نظارت در اپ اندروید):**

- ماژول جدید `feature:monitoring` دقیقاً با الگوی مورد ۲۱ ساخته شد: `MonitoringRepository`/`MonitoringRepositoryImpl` (unwrap پاسخ + throw روی خطا برای GET، `AcknowledgeResult` دوحالته برای عملیات نوشتنی که پیام خطای سرور را حفظ می‌کند)، `MonitoringViewModel` با `StateFlow<MonitoringUiState>`، `MonitoringScreen` (Compose)، `di/MonitoringModule.kt`. چهار endpoint جدید (`getMonitoringSummary`/`getMonitoringEvents`/`acknowledgeMonitoringEvent` + `ApiV2Routes.monitoringEventAcknowledge`) به `ApiServiceV2.kt` موجود (نه فایل جدید) اضافه شدند، هم‌الگوی دقیق متدهای مشابه.
- **مسیر ورود به فیچر، نه bottom nav/drawer** — این اپ هیچ‌کدام را ندارد؛ ناوبری یک `NavHost` + یک «شبکه منوی دسته‌بندی‌شده» در صفحه‌ی خانه است که آیتم‌هایش بر اساس `userPermissions["<key>"] == true` فیلتر می‌شوند (`getMenuItemsForUserType` در `feature:home`). آیتم مانیتورینگ پشت `view_monitoring` (همان permission ساخته‌شده در فاز الف) اضافه شد، به‌عنوان یک **مقصد ناوبری واقعی** (`MonitoringRoute`, هم‌الگوی `ManageShipsRoute` در feature:reports) نه دیالوگ (برخلاف بخش مدیریت کاربران که دیالوگ است) — چون این یک صفحه‌ی مستقل با لیست/فیلتر/بازخوانی است، نه یک فرم کوچک.
- به همین دلیل `feature:home` به `feature:monitoring` هم وابسته شد (دقیقاً همان الگوی وابستگی‌اش به `feature:reports`/`feature:admin` برای همین منظور) و `composable<MonitoringRoute>` در `MainScreen.kt` (اپ) کنار بقیه‌ی مقصدها ثبت شد.
- **بازخوانی خودکار هر ۳۰ ثانیه** در `MonitoringViewModel` (`while (isActive) { refresh(); delay(30_000) }` در `viewModelScope`، شروع/توقف با `DisposableEffect` در Composable) — هم‌الگوی کیفی حلقه‌ی polling موجود `ReportsViewModel.startRealTimePolling` (بدون شمارش‌معکوس ثانیه‌ای که آنجا برای UI مخصوص دیالوگ لحظه‌ای لازم بود، اینجا زائد بود) و هم‌ارز دقیق بازخوانی ۳۰ ثانیه‌ای داشبورد وب (فاز ب) — سه سطح (بک‌اند pull-based، داشبورد وب، اپ) اکنون رفتار یکسانی دارند.
- آیکون اختصاصی برای منوی مانیتورینگ نساخته شد (فقط PNGهای موجود در `feature:home/res/drawable`، بدون ابزار تولید آیکون در دسترس) — موقتاً از `ic_reports.png` استفاده شد؛ در کامنت کد علامت‌گذاری شد که یک جایگزین موقت است.
- کامپوننت‌های آماده‌ی `core:designsystem` (`StatisticsCard`, `EmptyState`, `ErrorState`, `LoadingOverlay`, `SemanticColors`) مستقیماً استفاده شدند، نه بازسازی — چهار کارت آمار (سلامت سیستم/بحرانی/هشدار/اطلاعاتی) از `StatisticsCard` موجود ساخته شدند.
- تأیید شد: `compileDebugKotlin`/`testDebugUnitTest` کل پروژه (شامل ماژول جدید) بدون خطا؛ ۵ تست جدید `MonitoringViewModelTest` (مسیر موفق refresh، خطای شبکه بدون کرش، تغییر فیلتر وضعیت، تأیید موفق با refresh خودکار بعدی، تأیید ناموفق با پیام سرور بدون refresh) سبز.
...
| ۳۸ | یکسان‌سازی دسترسی دیتابیس روی PDO و حذف `getMysqliConnection` | High | ✅ اعمال شد |
...
| ۴۱ | مهاجرت شبکه از Gson به `kotlinx.serialization` | High | ⏭️ فعلاً رد شد |
...
**یادداشت‌های اجرای مورد ۳۸:**

- بررسی اولیه‌ی گزارش «تنها `CargoRepository` مانده» نادرست بود — این یک یافته‌ی truncate‌شده‌ی graphify بود که با grep مستقیم اصلاح شد. دامنه‌ی واقعی ۱۵ فایل بود: `Database.php`/`DatabaseManager.php` (زیرساخت مشترک)، `CargoRepository`/`CargoService` (مسیر حیاتی ورود/خروج با تراکنش دستی و قفل ردیف `FOR UPDATE`)، چهار کنترلر (`AnalyticsController`, `CargoController`, `ChatController`, `UtilityController`) که مستقیماً `getMysqliConnection()` را صدا می‌زدند، `PermissionRepository`/`AuditLogger`/`MonitoringEventLogger`/`QuotaService`/`ShipService` (همه از طریق `DatabaseManager`)، `PasswordGateService`، و در نهایت `PermissionManager.php` (پنل ریشه) که در بررسی اول نیز از قلم افتاده بود. طبق تصمیم صریح کاربر پس از این تصحیح، هر ۱۵ فایل مهاجرت شدند.
- `Database::getPdoConnection()` اکنون همان `SET SESSION sql_mode = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION'` و `SET time_zone = '+03:30'` مسیر mysqli قبلی را روی اتصال PDO هم اجرا می‌کند (با دو `exec()` جدا، نه ترکیب‌شده با `SET NAMES` در یک دستور — MySQL اجازه‌ی ترکیب `SET NAMES` با سایر assignmentها در یک `SET` را نمی‌دهد) تا مهاجرت مصرف‌کننده‌ها رفتار خاموش این دو تنظیم را از دست ندهد.
- **الگوی تبدیل یکنواخت در هر ۱۵ فایل:** `bind_param("types", ...)` → `execute([...])` (بدون نیاز به type-string، چون PDO با آرایه bind می‌کند)، `get_result()->fetch_assoc()` → `fetch(PDO::FETCH_ASSOC)`، `fetch_all(MYSQLI_ASSOC)` → `fetchAll(PDO::FETCH_ASSOC)`، `->close()` حذف شد (PDOStatement نیازی ندارد)، `affected_rows`/`num_rows` → `rowCount()` / بررسی `fetch() !== false`، `autocommit(FALSE)` → `beginTransaction()`، `rollback()` → `rollBack()` (کیس صحیح PDO؛ فراخوانی متد در PHP به‌هرحال case-insensitive است)، `insert_id` (مسqli_stmt) → `$conn->lastInsertId()` (بعد از execute، نه روی خود statement).
- **یافته‌ی حین تبدیل که می‌توانست باگ واقعی بسازد:** در `ChatController::getMessages`، دو کوئری از `LIMIT ?` استفاده می‌کردند. با `PDO::ATTR_EMULATE_PREPARES => false` (که این پروژه از قبل فعال دارد)، MySQL's real prepared statements پارامتر رشته‌ای (رفتار پیش‌فرض bind آرایه‌ای PDO) در بند `LIMIT` را نمی‌پذیرد و با خطای syntax رد می‌شود — mysqli این مشکل را نداشت چون `bind_param("...i...")` نوع را صریح int می‌فرستاد. رفع شد با درج مستقیم `$limit` (که در امضای تابع از نوع PHP `int` است، نه رشته‌ی کاربر) در متن کوئری به‌جای placeholder — بدون ریسک SQL injection چون نوع در سطح PHP تضمین‌شده است.
- **معناشناسی حیاتی حفظ‌شده در `CargoRepository`:** `updateCargoExit`/`updateCargoShortageOrExcess` تشخیص تداخل هم‌زمانی (۴۰۹ Conflict) را با «۰ ردیف تحت‌تأثیر» می‌سنجند. `PDOStatement::rowCount()` برای UPDATE/DELETE با درایور MySQL دقیقاً همان معنایی را دارد که `mysqli_stmt::affected_rows` داشت (تعداد ردیف‌های واقعاً تغییریافته، نه صرفاً منطبق‌شده — چون `PDO::MYSQL_ATTR_FOUND_ROWS` در هیچ‌کدام تنظیم نشده)، پس رفتار تشخیص تداخل بدون تغییر باقی ماند.
- `CargoService::saveOrUpdateCargo` (تنها جای دارای تراکنش دستی چندمرحله‌ای در کل کدبیس): ترتیب `SET SESSION` قبل از `beginTransaction()` عمداً حفظ شد — این دو تنظیم session-level هستند نه transaction-scoped، پس ترتیب نسبت به شروع تراکنش بی‌اثر است؛ `findCargoByKeys` (با `FOR UPDATE`) هنوز داخل try block بعد از `beginTransaction()` فراخوانی می‌شود، پس قفل ردیف در تراکنش واقعی اعمال می‌شود.
- `getMysqliConnection()`/`closeConnections()`‌ی مربوط به mysqli از `Database.php` کاملاً حذف شدند؛ `grep -rl "mysqli"` روی کل `PHP/` (به‌جز `vendor/` و کش `build/phpstan/`) صفر نتیجه می‌دهد.
- تأیید شد: `php -l` روی هر ۱۵ فایل، بارگذاری کامل `routes/api_v2.php` بدون خطای fatal، `vendor/bin/phpstan analyse` بعد از regenerate باسلاین (برای حذف ~۸۰ رکورد یتیم مربوط به شکل قدیمی mysqli-typed، از ۴۶۲ به ۳۴۱ خطا — کاهش واقعی چون typing واقعی PDO دقیق‌تر از mysqli بود) «No errors» می‌دهد، و کل مجموعه‌ی PHPUnit (۱۱۲ تست) با ۱ شکست نامرتبط (تست زمان‌سنجی `LoginAttemptLimiterTest` که با تأخیر شناخته‌شده‌ی «DB connection refused» این sandbox بدون MySQL محلی تیک می‌خورد، نه با این تغییر). تست end-to-end زنده (تراکنش هم‌زمان واقعی، قفل ردیف تحت بار) روی یک MySQL واقعی در این محیط ممکن نبود — قبل از deploy توصیه می‌شود مسیر ثبت/خروج حواله به‌خصوص با دو درخواست هم‌زمان روی یک حواله دستی تست شود.
