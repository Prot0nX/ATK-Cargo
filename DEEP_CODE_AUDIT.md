...
| مورد | اندازه | وضعیت |
|------|--------|--------|
| `graphify-out/` | **۹۱ مگابایت، ۴۷ فایل tracked** | اسنپ‌شات‌های روزانه‌ی گراف در گیت کامیت شده‌اند و روزانه رشد می‌کنند |
| `java_pid23540.hprof` | **۱٫۶۹ گیگابایت** | heap dump در working tree (gitignore شده، ولی روی دیسک) |
| `php_server_info_20260819_104131.txt` | ۳۵ کیلوبایت | dump اطلاعات سرور (gitignore شده) |
| `whimsical-beaming-starfish.md` | ۷ کیلوبایت | فایل بی‌نام‌ونشان در ریشه |
| `.claude/worktrees/eager-curran-a89d37/` | یک checkout کامل | کپی تکراری پروژه (untracked) |
| `DEEP_CODE_REVIEW.md` | حذف‌شده در working tree | گزارش ممیزی قبلی که هنوز در ده‌ها کامنت کد به آن ارجاع داده می‌شود |

**Recommended Fix:**
```gitignore
# در .gitignore اضافه شود
/graphify-out/
.claude/worktrees/
```
و تاریخچه‌ی `graphify-out/` را با `git filter-repo` پاک کن. فایل‌های موقت روی دیسک را حذف کن.

> **نکته:** ده‌ها کامنت در کد به `DEEP_CODE_REVIEW.md` و `DEEP_CODE_AUDIT.md #PhaseX` ارجاع می‌دهند. حالا که این گزارش جایگزین شده، یا آن فایل را در ریپازیتوری نگه دار (برای اینکه ارجاع‌ها معنادار بمانند) یا ارجاع‌ها را به این فایل به‌روز کن.
