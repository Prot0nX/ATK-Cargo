...
| 4.3 | ⚠️ در حال انجام — `feature:auth` (5.10)، `feature:startup` + `core:domain` (5.11)، `feature:admin` (5.12) استخراج شدند؛ ۹ feature باقی‌مانده هنوز در `app` هستند و اکثرشان (`cargo_*`, `reports`, `chat`) به‌شدت به‌هم و به `CargoViewModel`/`ui/viewmodel` (God ViewModel) وابسته‌اند — کشف شد حین بررسی `cargo_details` به‌عنوان کاندید (رد شد). جزئیات در پایین | High |
