...
| 4.3 | ⚠️ **متوقف در یک نقطه‌ی پایدار** — ۴ ماژول feature استخراج شد: `feature:auth` (5.10)، `feature:startup` + `core:domain` (5.11)، `feature:admin` (5.12)، `feature:chat` (5.13). ۸ مورد باقی‌مانده (`cargo`, `cargo_counter`, `cargo_details`, `cargo_entry`, `cargo_registration`, `home`, `reports`, `update`) **با همین الگو قابل‌استخراج نیستند** — یک مانع کیفی متفاوت کشف شد، نه فقط حجم بیشتر: `home`, `cargo_counter`, `cargo_details`, `cargo_entry`, `cargo_registration`, `reports` یک **حلقه‌ی وابستگی واقعی** تشکیل می‌دهند (`home` تابع ناوبری `cargo_counter` را import می‌کند، `cargo_counter` هم تابع ناوبری `home` را — و مشابه آن بین `cargo_details`/`cargo_entry`/`reports`) چون هر feature مستقیماً تابع‌های `navigateToX` و کامپوزبل صفحه‌ی feature دیگر را import می‌کند، نه از طریق یک لایه‌ی ناوبری مشترک. ماژول‌های Gradle باید DAG باشند (بدون حلقه)، پس این ۶ مورد را نمی‌شود بدون بازطراحی معماری ناوبری (مثلاً هر feature فقط route خودش را expose کند — همان الگوی `@Serializable object AdminChatRoute` که برای `feature:chat` استفاده شد — و سیم‌کشی واقعی بین صفحات در یک NavGraph مرکزی در `app` انجام شود) جدا کرد؛ این یک بازطراحی معماری است، نه move-file. **تصمیم کاربر: همین‌جا متوقف شود** — این یافته صرفاً مستند شد، ادامه‌ی Phase 4.3 برای این ۶ مورد به تصمیم بعدی موکول شد. `update` و `cargo` (منطق خالص) هم بررسی نشدند چون هدف روشن‌کردن مسیر کلی بود، نه تکمیل مکانیکی باقی‌مانده. | High |
...
| 4.10 | ⚠️ چک‌لیست به تست عملی روی throwaway DB محلی ارتقا یافت (Phase 5.15) — ریسک واقعی کشف‌شده (بدون سقف طول روی فیلدهای متنی آزاد) رفع شد (Phase 5.16). فعال‌سازی نهایی `Database.php:77` روی production طبق تصمیم صریح کاربر هنوز انجام نشده — منتظر گام ۱ (اسکن داده‌ی واقعی تولید، نیاز به دسترسی کاربر) است. جزئیات در `PHP/STRICT_MODE_CHECKLIST.md` | Medium |
...
| 5.14 | ⚠️ بررسی ۸ feature باقی‌مانده‌ی Phase 4.3 — نقشه‌ی import بین‌featureای کامل کشیده شد. نتیجه: `home`↔`cargo_counter` مستقیماً یکدیگر را import می‌کنند (چرخه)؛ `cargo_counter`→`cargo_details`/`cargo_entry`/`reports`، `cargo_details`→`cargo_entry`/`cargo_registration`/`home`، `reports`→`cargo_details` هم به همین حلقه متصل‌اند. این ۶ مورد را با الگوی move-file/interface-boundary که برای ۴ ماژول قبلی جواب داد نمی‌شود جدا کرد؛ نیاز به معماری ناوبری مبتنی بر route (نه import مستقیم تابع/کامپوزبل feature دیگر) دارد. طبق تصمیم کاربر، این کار به بعد موکول و فقط همین یافته مستند شد — کدی تغییر نکرد. | High |

| 5.15 | ⚠️ ادامه‌ی Phase 4.10 — گام‌های ۲ و ۴ چک‌لیست `STRICT_MODE_CHECKLIST.md` روی یک دیتابیس MariaDB throwaway محلی (ساخته‌شده از خودِ `schema.sql`، بدون داده‌ی واقعی تولید — چون به تولید دسترسی نیست) واقعاً اجرا شد؛ یک اسکریپت PHP مسیرهای نوشتن واقعی (`CargoRepository::insertCargo`, INSERT مستقیم `InitialInfo`/`user_sessions`, `PermissionRepository::saveUserPermissions`) را با مقادیر عادی و مرزی زیر `STRICT_TRANS_TABLES` اجرا کرد. **یافته‌ی واقعی:** `CargoController::validateStringField` سقف طول ندارد — امروز رشته‌ی بلندتر از ستون `varchar` بی‌صدا truncate می‌شود، بعد از فعال‌سازی strict همان ورودی ۵۰۰ می‌دهد؛ در عمل بعید برای `scaleReceiptNumber` ولی دقیقاً همان الگوی ریسکی که گزارش هشدار داده بود. **یافته‌ی دوم:** PDO (در `SessionRepository`/`UserRepository`) هیچ `sql_mode`ای صریح تنظیم نمی‌کند و از global سرور تبعیت می‌کند، در حالی که مسیر mysqli صراحتاً غیر-strict است — یعنی وضعیت strict/non-strict این دو مسیر روی production ممکن است از قبل متفاوت باشد (نیاز به تأیید `SELECT @@GLOBAL.sql_mode` روی سرور واقعی). فعال‌سازی نهایی (تغییر `Database.php:77`) هنوز انجام نشده — منتظر تصمیم کاربر درباره‌ی افزودن سقف طول و اجرای گام ۱ (اسکن داده‌ی تولید واقعی، که در این محیط ممکن نیست) است. جزئیات کامل در `PHP/STRICT_MODE_CHECKLIST.md` | Medium |

| 5.16 | ✅ رفع ریسک کشف‌شده در Phase 5.15 — سقف طول ۱۰۰ کاراکتری (برابر `varchar(100)` تمام ستون‌های متنی `CargoInfo`) به دو مسیر نوشتن اضافه شد: `CargoController::validateStringField` (پارامتر جدید `?int $maxLength = 100`) برای مسیر `updateCargoInfo`، و چک صریح `mb_strlen` + `sendErrorResponse` در `CargoController::saveOrUpdate` (مسیر اصلی ثبت حواله) بعد از `sanitizeString`. این تغییر مستقل از فعال‌سازی نهایی strict mode مفیده — همین امروز هم جلوی truncate بی‌صدا رو می‌گیره. `php -l`، `phpstan` (صفر خطا)، هر ۶۸ تست PHPUnit سبز | Low |

| 5.17 | 🐛 **باگ واقعی کشف و رفع شد (خارج از دامنه‌ی برنامه‌ریزی‌شده، گزارش کاربر)** — تب‌های «تائید نشده/تائید شده» در `CargoDetailsScreen` (صفحه‌ی «نظارت بارشمار») بعد از ثبت حواله‌ی جدید خالی نمایش داده می‌شدند با اینکه سرور لیست کامل را درست برمی‌گرداند (تأیید شد با لاگ شبکه‌ی کاربر). علت: `groupedCargoList` با `remember { derivedStateOf {...} }` **بدون کلید** محاسبه می‌شد، در حالی که بدنه‌ی آن فقط یک `val filteredCargoInfoList` معمولی (نه یک Compose `State`) را closure می‌کرد — طبق مستندات Compose، `derivedStateOf` فقط زمانی خودکار ردیابی می‌کند که داخل block خودش یک `State.value` بخواند؛ اینجا چنین چیزی نبود، پس این محاسبه فقط یک‌بار (در اولین composition، وقتی لیست هنوز خالی از پاسخ شبکه بود) اجرا و برای همیشه cache می‌شد — کامنت کد هم همین برداشت غلط را توضیح داده بود. رفع: `remember(filteredCargoInfoList) {...}` بدون `derivedStateOf` (همان الگوی درستی که `sortedUnconfirmed`/`sortedConfirmed`/`confirmedStats` در `CargoListSection` همان فایل از قبل استفاده می‌کردند). این باگ از **Phase 3 (کامیت `938867b`)** در کد بوده، ربطی به تغییرات اخیر این فاز نداشت. بررسی شد که ۳ محل دیگر `derivedStateOf` در پروژه (`ShipsListScreen`, `VoucherDetailsDialogSection`, `ChatScreen`) این الگوی غلط را ندارند — یا کلید صریح دارند یا واقعاً `State.value` (مثل `LazyListState.layoutInfo`) می‌خوانند. `compileDebugKotlin` کامل + `assembleDebug` + هر ۴۷ تست سبز | High |

| 5.18 | 🐛 **باگ واقعی دوم کشف و رفع شد (گزارش کاربر)** — در صفحه‌ی «جزئیات کشتی»، بعد از فعال/غیرفعال کردن یک کوتاژ، لیست کوتاژها و خلاصه‌ی انبارها در UI به‌روز نمی‌شدند با اینکه سرور واقعاً وضعیت را عوض کرده بود. علت **کش HTTP سمت کلاینت** بود، نه یک باگ Compose: `getShipDetails`/`getShipQuotas` با `Cache-Control: max-age=6` از سرور می‌آیند (برای بار عادی/polling مفید) — OkHttp طبق آن تا ۶ ثانیه بعد از آخرین fetch حتی یک درخواست شبکه هم نمی‌زند و پاسخ کش‌شده‌ی دیسک را مستقیم برمی‌گرداند. `ReportsViewModel.refreshShipDataSilently` (که بعد از هر نوشتن موفق — toggleQuotaStatus، editQuota، deleteQuota، updateQuotaPercentage، deactivateQuotasInBulk — صدا زده می‌شود) این دو متد را بدون هیچ مکانیزم دور زدن کش صدا می‌زد؛ روی یک تعامل سریع (که معمول‌ترین حالت است)، این رفرش همان پاسخ قبل از نوشتن را از کش دیسک می‌گرفت. رفع: پارامتر اختیاری `@Header("Cache-Control") cacheControl` به `ApiServiceV2.getShipDetails`/`getShipQuotas` اضافه شد؛ `ReportsRepository`/`refreshShipDataSilently` با `forceRefresh=true` مقدار `"no-cache"` را ارسال می‌کنند — این جزء ۷۲۳۴ RFC (درخواست‌دهنده) OkHttp را مجبور به یک درخواست شرطی واقعی (`If-None-Match`) می‌کند، بدون آنکه کش عادی (بار اول صفحه، polling) دست‌نخورده حذف شود. **یافته‌ی مرتبط، رفع‌نشده:** `loadShips()` (لیست کشتی‌ها، `Cache-Control: max-age=8`) هم بعد از همین نوشتن‌ها صدا زده می‌شود و همان الگوی ریسک را دارد؛ چون کاربر مشکل را فقط در جزئیات کشتی گزارش کرد و صفحه‌ی لیست کشتی‌ها تأیید نشده بود، عمداً دست‌نخورده ماند — احتمالاً نیاز به همین رفع دارد. `compileDebugKotlin` کامل + `assembleDebug` + هر ۴۷ تست سبز | High |

| 5.19 | 🐛 **باگ واقعی سوم کشف و رفع شد (گزارش کاربر)** — عملیات کوتاژ (فعال/غیرفعال، ویرایش، درصد، حذف، تناژ موقت) در `audit_log` ثبت نمی‌شدند؛ ویرایش کاربر در «مدیریت کاربران» درست ثبت می‌شد. علت: `AppApiController::$authenticatedUsername` فقط داخل `handle()` (از طریق `AuthenticatesRequests::requireAuthenticatedSession()`) پر می‌شد، اما `handle()` از حذف `app_api.php` (v1) در Phase 3.1 دیگر از هیچ‌جا صدا زده نمی‌شود — روتر v2 مستقیماً متدهای `editQuota`/`toggleQuotaStatus`/`updateQuotaPercentage`/`updateQuotaPercentageRestriction`/`deleteQuota`/`updateTemporaryTonnage` را صدا می‌زند و از `handle()` رد نمی‌شود. نتیجه: `$this->authenticatedUsername` همیشه `null` می‌ماند → شرط `actorUsername !== null` در `QuotaService` هیچ‌وقت true نمی‌شد → `AuditLogger::log` بی‌صدا هیچ‌وقت اجرا نمی‌شد (خودِ `AuditLogger` هم همه‌ی استثناها را می‌بلعد، پس حتی خطایی هم دیده نمی‌شد). `UserController`/`ChatController` چون از الگوی متفاوتی استفاده می‌کنند (`handle()`شان واقعاً صدا زده می‌شود و خودش auth را چک می‌کند) درگیر این باگ نبودند. رفع: افزودن `$this->requireAuthenticatedSession();` به ابتدای هر ۶ متد نوشتنی — دقیقاً همان الگوی «بررسی دوگانه‌ی واقعی» که برای `CargoController::saveOrUpdate` (که همین فراخوانی را در همه‌ی ۱۰ متد نوشتنی‌اش دارد) در `routes/api_v2.php` مستند شده. `php -l`، `phpstan analyse` (صفر خطا)، و کل ۶۸ تست PHPUnit سبز. **هنوز روی سرور production تأیید نشده** — نیاز به deploy مجدد `AppApiController.php` و تست واقعی دارد. | Medium |
